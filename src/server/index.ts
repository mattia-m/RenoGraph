...
app.post("/api/renovations/:id/professionals", route(async (request, response) => response.status(201).json(await getStore(request).addProfessional(request.body))));
app.post("/api/renovations/:id/assignments", route(async (request, response) => response.status(201).json(await getStore(request).assignProfessional(request.body?.taskId, request.body?.professionalId))));
app.delete("/api/renovations/:id/assignments/:assignmentId", route(async (request, response) => { await getStore(request).removeAssignment(String(request.params.assignmentId)); response.status(204).end(); }));
app.post("/api/renovations/:id/contractors", route(async (request, response) => response.status(201).json(await getStore(request).addContractor(request.body))));
app.post("/api/renovations/:id/purchases", route(async (request, response) => response.status(201).json(await getStore(request).addPurchase(request.body))));
app.patch("/api/renovations/:id/purchases/:purchaseId", route(async (request, response) => response.json(await getStore(request).updatePurchase(String(request.params.purchaseId), request.body?.status))));
app.post("/api/renovations/:id/documents", route(async (request, response) => response.status(201).json(await getStore(request).addDocument(request.body))));
...
app.patch("/api/renovations/:id/nodes/:nodeId", route(async (request, response) => response.json(await getStore(request).updateNode(String(request.params.nodeId), nodePatch(request.body)))));
app.patch("/api/renovations/:id/nodes/:nodeId/options/:optionId", route(async (request, response) => response.json(await getStore(request).updateMaterialOption(String(request.params.nodeId), String(request.params.optionId), optionPatch(request.body)))));
app.post("/api/renovations/:id/nodes/:nodeId/select-option", route(async (request, response) => { if (typeof request.body?.optionId !== "string") throw new Error("MATERIAL_OPTION_NOT_FOUND"); response.json(await getStore(request).selectMaterialOption(String(request.params.nodeId), request.body.optionId)); }));
...
for (const [action, status] of [["start", "IN_PROGRESS"], ["complete", "COMPLETED"], ["block", "BLOCKED"]] as const) {
  app.post(`/api/renovations/:id/nodes/:nodeId/${action}`, route(async (request, response) => response.json(await getStore(request).transition(String(request.params.nodeId), status as NodeStatus, request.body?.actualDurationDays))));
}
