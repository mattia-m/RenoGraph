...
export function validateScenarioInput(input: unknown): { name: string; changes: ScenarioChange[] } {
  ...
  const changes = candidate.changes.map((raw) => {
    ...
    for (const key of ["durationDeltaDays", "newDurationDays", "estimatedCostDelta", "deliveryDeltaDays", "newDeliveryDays", "actualDurationDays"]) {
    ...
    if (change.actualDurationDays !== undefined && (change.actualDurationDays as number) < 0) throw new Error("INVALID_DURATION");
