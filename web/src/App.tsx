...
import type { GraphNode, GraphResponse, MaterialOption, NodeStatus, Relationship, RoomMaterialRequirement, ScenarioResult, Summary } from "../../src/shared/types.js";
...
function RenovationNodeCard({ data }: NodeProps<FlowNode>) {
  ...
  return <>
    ...
    <button className={`graph-card ${data.type.toLowerCase()} ${data.status.toLowerCase()} ${data.critical ? "critical" : ""}`} onClick={() => data.onSelect?.(data)}>
      ...
      <span className="card-meta">{data.durationDays ? `${data.durationDays} days` : data.type === "MATERIAL" ? `${selectedDeliveryDays(data)}d delivery` : "space"}{data.estimatedCost ? ` · ${money.format(data.estimatedCost)}` : ""}</span>
...
function EditPanel({ node, allNodes, dependencyEdges, busy, onClose, onSaveNode, onSaveOption, onSelectOption, onAddDependency, onRemoveDependency }: { node: GraphNode; allNodes: GraphNode[]; dependencyEdges: Array<Relationship & { source: string; target: string }>; busy: boolean; onClose: () => void; onSaveNode: (id: string, patch: Record<string, unknown>) => Promise<void>; onSaveOption: (id: string, optionId: string, patch: Record<string, unknown>) => Promise<void>; onSelectOption: (optionId: string) => void; onAddDependency: (from: string, to: string, type: "DEPENDS_ON" | "REQUIRES_MATERIAL") => Promise<void>; onRemoveDependency: (id: string) => Promise<void> }) {
  ...
  const save = async () => {
    await onSaveNode(node.id, { name, description, status, ...(node.type === "TASK" ? { durationDays: duration, estimatedCost: cost } : {}) });
...
function RuntimeInspector({ node, graph }: { node: GraphNode; graph?: GraphResponse }) {
  ...
  const ownKeys = node.type === "TASK" ? [`${node.id}__completed`, `${node.id}__in_progress`, `${node.id}__ready`, `${node.id}__state`] : node.type === "MATERIAL" ? [`${node.id}__delivered`, `${node.id}__option`, `${node.id}__available`] : [`${node.id}__materials`];
